...
public class StaffOut extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
					...
					for(int i=0 ; i<SignUp.getStaff().size();i++)
				{
					if ( (textField).equals(SignUp.getStaff().get(i).getUser()) && (textField_1).equals(SignUp.getStaff().get(i).getPassword()))
					{
						...
						double Tprice = (stop - StaffIn.getTime().get(i))*price;
						
						JOptionPane.showMessageDialog(null, "Total Price is :  "+ Tprice+"\n "+SignUp.getStaff().get(i).getName()+" Logged out ");
						setVisible(false);
						StaffIn.getStaff().remove(i);
						StaffIn.getTime().remove(i);
